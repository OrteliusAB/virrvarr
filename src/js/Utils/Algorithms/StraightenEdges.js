 ...
 * @param {VVNode[][]} fullHierarchy
 * @param {VVEdge[] | {sourceNode: string, targetNode: string}} edgesIn
 * @returns {VVNode[][]|null} - Ordered Hierarchy
 */
export const straightenEdges = (fullHierarchy, edgesIn) => {
	//Prepare the data
	...
	return fullHierarchy.map(level => level.map(node => (node instanceof VVNode ? node : null)))
}
