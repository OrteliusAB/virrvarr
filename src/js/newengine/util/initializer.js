...
export const initializeNodesAndEdges = (nodes = [], edges = []) => {
	...
	for (let i = 0; i < edges.length; i++) {
		...
		if (!edge.distance) {
			const targetRadius = edge.target.radius !== undefined ? edge.target.radius : 0
			const sourceRadius = edge.source.radius !== undefined ? edge.source.radius : 0
			const invisibleDistance = targetRadius + sourceRadius
