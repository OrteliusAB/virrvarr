...
export default class EntityProcessor {
	 ...
	 */
	getSelection() {
		const selectedNodes = this.datastore.nodes
			.filter(node => node.isFocused)
			.map(node => {
				return { id: node.id, type: "node", data: node.data }
			})
		const selectedEdgeTo = this.datastore.edges
			.filter(edge => edge.isToFocused)
			.map(edge => {
				return { id: edge.id, type: "edge", data: edge.data, direction: "to" }
			})
		const selectedEdgeFrom = this.datastore.edges
			.filter(edge => edge.isFromFocused)
			.map(edge => {
				return { id: edge.id, type: "edge", data: edge.data, direction: "from" }
			})
		return [...selectedNodes, ...selectedEdgeTo, ...selectedEdgeFrom]
	}
