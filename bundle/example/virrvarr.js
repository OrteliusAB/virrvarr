...
(function (global, factory) {
  ...
  var EVENTS = Object.freeze({
    ...
    // Params: [...nodeIDs]
    FADE_NODE_REQUESTED: "FADE_NODE_REQUESTED_EVENT",
    // Params: N/A
    CLEAR_FADE_NODE_REQUESTED: "CLEAR_FADE_NODE_REQUESTED_EVENT",
    // Params N/A
  ...
  var tween = function tween(element, property, initialValue, target, startTime, animationTime) {
    ...
    var newValue = initialValue + (target - initialValue) * percentOfAnimation;
    element.style[property] = newValue;
    setTimeout(function () {
      return tween(element, property, initialValue, target, startTime, animationTime);
    }, 1);
  };
  ...
  /*#__PURE__*/
  function () {
    function Highlighter(eventEmitter, userDefinedOptions) {
      var _this = this;
      ...
      this.ee.on(EVENTS.FADE_NODE_REQUESTED, function (nodeIDs) {
        _this.fadeNodes(nodeIDs);
      });
      this.ee.on(EVENTS.CLEAR_FADE_NODE_REQUESTED, function () {
        _this.clearFade();
      });
    ...
    _createClass(Highlighter, [{
      ...
      value: function toggleOnionBorder(DOMElement, size, color) {
        ...
        if (!found) {
          ...
          for (var i = 1; i < layers + 1; i++) {
            ...
            CSSUtil.tween(clone, "scale", DOMElement.getBoundingClientRect().width / clone.getBoundingClientRect().width, 1, Date.now(), Env.DEFAULT_ONION_ANIMATION_TIME * i);
            previousNode = clone;
          }
      ...
      key: "fadeNodes",
      value: function fadeNodes(nodeIDsToFade) {
        selectAll(".node").filter(function (d) {
      ...
      value: function clearFade() {
        selectAll(".faded").classed("faded", false);
      }
    }]);
    ...
    _createClass(DOMProcessor, [{
      ...
      value: function drawNodeCollapsedEdgeCounter(element, data) {
        ...
        var rightTopRoundedRect = function rightTopRoundedRect(x, y, width, height, radius) {
          return "M" + x + "," + y + "h" + (width - radius) + "q" + radius + ",0 " + radius + "," + radius + "v" + (height - 2 * radius) + "q" + "0," + radius + " " + -radius + "," + radius + "h" + (radius * 2 - width) + "q" + -radius + ",0 " + -radius + "," + -radius + "z";
        };

        element.append("g").attr("id", "badge-" + data.id + "-hidden-edge-counter").attr("style", "pointer-events:none;").attr("transform", "translate(".concat(translateX, " ").concat(translateY, ")")).append("path").attr("d", rightTopRoundedRect(-(rectWidth / 2), -(rectHeight / 2), rectWidth, rectHeight, 10)).attr("class", "virrvarr-node-edge-counter-badge").select(function () {
          return this.parentNode;
  ...
  function () {
    ...
    _createClass(Virrvarr, [{
      ...
      key: "fade",
      value: function fade(attribute, value, filterFunction) {
        if (attribute && value || filterFunction) {
          var nodesToFade = this._datastore.nodes.filter(function (node) {
            if (filterFunction) {
              return filterFunction(node.data);
            }
          ...
          this._ee.trigger(EVENTS.FADE_NODE_REQUESTED, nodesToFade);
        } else {
          throw new Error("No attribute, value or filterfunction provided");
        }
      }
      ...
      key: "clearFade",
      value: function clearFade() {
        this._ee.trigger(EVENTS.CLEAR_FADE_NODE_REQUESTED);
      }
      /**
