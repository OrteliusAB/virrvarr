...
(function (global, factory) {
  ...
  function () {
    function VVNode(id, type, name, icon, data) {
      ...
      this.x = undefined;
      this.y = undefined; //Fixed coordinates
      ...
      this.radius = null;
      this.width = null;
      this.height = null; //Animation utility properties
  ...
  var VVEdge = function VVEdge(id, type, sourceNode, targetNode, nameFrom, nameTo, multiplicityFrom, multiplicityTo, data) {
    ...
    this.data = data; //Visual information

    this.isFiltered = false;
    this.isHidden = false; //Meta Data calculated at runtime
  ...
  function () {
    ...
    _createClass(Highlighter, [{
      ...
      value: function removeAllEntityFocus() {
        select(this.graphContainerElement).selectAll(".focused").classed("focused", false);

        if (this.enableOnionOnFocus) {
          select(this.graphContainerElement).selectAll(".onion-clone").attr("class", "removing").transition().duration(Env.DEFAULT_ONION_ANIMATION_TIME).style("transform", "scale(0.8)").remove();
        }
      }
      /**
